...
class SubmissionBase:
    ...
    def grade(self):
        ...
        for part_id, result in self:
            parts[self.part_names_key[part_id - 1]] = {
        ...
        for index, part in enumerate(parts):
            part_feedback = response["linked"][
                "onDemandProgrammingScriptEvaluations.v1"
            ][0]["parts"][str(part)]["feedback"]
            part_evaluation = response["linked"][
                "onDemandProgrammingScriptEvaluations.v1"
            ][0]["parts"][str(part)]
            score = "%d / %3d" % (part_evaluation["score"], part_evaluation["maxScore"])
            print(
                "%43s | %9s | %-s" % (self.part_names[int(index)], score, part_feedback)
            )
    ...
    def login_prompt(self):
        if os.path.isfile(self.save_file):
            ...
            reenter = input(
                "Use token from last successful submission (%s)? (Y/n): " % login
            )
